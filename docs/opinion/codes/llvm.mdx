...
Q: rust (via rustc) zig and c (via clang) hit llvm apis to produce ir, which llvm compiles to machine code. theoretically, any language compiling to llvm ir can link its binary with any other one. 

llvm should compile not only to machine code but also wasm. it could be one of the modules.
